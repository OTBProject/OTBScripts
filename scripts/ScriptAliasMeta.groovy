...
private boolean add(ScriptArgs sArgs) {
    ...
    Alias alias = DefaultCommandGenerator.createDefaultAlias();
...
private boolean list(ScriptArgs sArgs) {
    ...
    if (sArgs.channel.equals(APIBot.getBot().getUserName())) {
        ...
        List<String> list = Aliases.getAliases(db).stream().sorted().collect(Collectors.toList());
        asString = "Bot Aliases: " + list.toString() + "; ";
    }
    List<String> list = Aliases.getAliases(sArgs.db).stream().sorted().collect(Collectors.toList());
